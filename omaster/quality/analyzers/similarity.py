...
class SimilarityAnalyzer(BaseAnalyzer):
    ...
    def __init__(self, project_path: Path, config: Dict[str, Any]):
        ...
        similarity_config = config.get("quality", {}).get("similarity", DEFAULT_CONFIG)
        self.min_lines = similarity_config.get("min_lines", DEFAULT_CONFIG["min_lines"])
        self.exact_match_threshold = similarity_config.get(
            "exact_match_threshold",
            DEFAULT_CONFIG["exact_match_threshold"]
        )
        self.ast_similarity_threshold = similarity_config.get(
            "ast_similarity_threshold",
            DEFAULT_CONFIG["ast_similarity_threshold"]
        )
        self.token_similarity_threshold = similarity_config.get(
            "token_similarity_threshold",
            DEFAULT_CONFIG["token_similarity_threshold"]
        )
        self.cfg_similarity_threshold = similarity_config.get(
            "cfg_similarity_threshold",
            DEFAULT_CONFIG["cfg_similarity_threshold"]
        )
        self.semantic_similarity_threshold = similarity_config.get(
            "semantic_similarity_threshold",
            DEFAULT_CONFIG["semantic_similarity_threshold"]
        )

        # Get metric weights with defaults
        self.weights = similarity_config.get("weights", DEFAULT_CONFIG["weights"])

        # Get severity weights with defaults
        self.severity_weights = similarity_config.get("severity_weights", DEFAULT_CONFIG["severity_weights"])
    ...
    def analyze(self) -> List[QualityIssue]:
        ...
        for blocks in exact_duplicates:
            ...
            for other in others:
                issues.append(QualityIssue(
                    type=QualityMetric.DUPLICATION,
                    file_path=other.file,
                    line=other.start_line,
                    end_line=other.end_line,
                    message=f"Code block is identical to block at {primary.file}:{primary.start_line}-{primary.end_line} ({other.size} lines)",
                    severity="high",
        ...
        for block1, block2, similarity in ast_similar:
            issues.append(QualityIssue(
                type=QualityMetric.AST_SIMILARITY,
                file_path=block2.file,
                line=block2.start_line,
                end_line=block2.end_line,
                message=f"Code block has {similarity:.1%} structural similarity with block at {block1.file}:{block1.start_line}",
                severity="medium",
        ...
        for block1, block2, similarity in token_similar:
            issues.append(QualityIssue(
                type=QualityMetric.TOKEN_SIMILARITY,
                file_path=block2.file,
                line=block2.start_line,
                end_line=block2.end_line,
                message=f"Code block has {similarity:.1%} token sequence similarity with block at {block1.file}:{block1.start_line}",
                severity="low",
        ...
        for block1, block2, similarity in cfg_similar:
            issues.append(QualityIssue(
                type=QualityMetric.CFG_SIMILARITY,
                file_path=block2.file,
                line=block2.start_line,
                end_line=block2.end_line,
                message=f"Code block has {similarity:.1%} control flow similarity with block at {block1.file}:{block1.start_line}",
                severity="low",
                weight=self.severity_weights["low"]
            ))
        ...
        for block1, block2, similarity in semantic_similar:
            issues.append(QualityIssue(
                type=QualityMetric.SEMANTIC_SIMILARITY,
                file_path=block2.file,
                line=block2.start_line,
                end_line=block2.end_line,
                message=f"Code block has {similarity:.1%} semantic similarity with block at {block1.file}:{block1.start_line}",
                severity="info",
                weight=self.severity_weights["low"]
            ))
